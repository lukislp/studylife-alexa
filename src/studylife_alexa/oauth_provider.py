...
Round trip:
  ...
  2. That redirects to StudyLife's own /connect/client/studylife-alexa (passkey login +
     consent happens entirely on StudyLife's side).
  3. StudyLife redirects back to GET /oauth/studylife/callback with a single-use
     assertion, which this server exchanges server-to-server for a StudyLife API key
     (client.py's exchange_assertion).
  4. This server mints its OWN opaque authorization code, stores it mapped to the
     (encrypted) API key, and redirects to Alexa's own redirect_uri.
  5. Alexa's backend calls POST /token server-to-server to exchange that code (or later,
     a refresh token) for this server's own opaque access/refresh tokens.
  6. Every subsequent skill request carries the access token in
     context.System.user.accessToken - handlers.py resolves it back to the underlying
     StudyLife API key via OAuthStore.load_access_token.
"""
...
from studylife_alexa.client import exchange_assertion
...
def register_oauth_routes(app: FastAPI, store: OAuthStore, settings: Settings) -> None:
    ...
    async def authorize(request: Request) -> Response:
        ...
        connect_url = (
            f"{str(settings.studylife_connect_url).rstrip('/')}"
            f"/connect/client/studylife-alexa?{query}"
        )
    ...
    async def studylife_callback(request: Request) -> Response:
        ...
        exchanged = await exchange_assertion(str(settings.studylife_base_url), assertion)
        ...
        await store.save_authorization_code(code, exchanged.api_key)
    ...
    async def token(request: Request) -> Response:
        ...
        if grant_type == "authorization_code":
            api_key = await store.consume_authorization_code(str(form.get("code", "")))
        elif grant_type == "refresh_token":
            api_key = await store.consume_refresh_token(str(form.get("refresh_token", "")))
        else:
        ...
        await store.save_refresh_token(refresh_token, api_key)
